...
type Props = {
  className?: string
  imgClassName?: string
  /** OS menubar / minimal: glyph only, no bordered tile */
  bare?: boolean
}

/** Monogram mark - inline SVG with `currentColor` (no img + invert halos). */
export function BrandLogo({ className, imgClassName, bare }: Props) {
  if (bare) {
    return (
      <span className={cn('inline-flex shrink-0 items-center justify-center', className)} aria-hidden>
        <svg
          ...
          className={cn(
            'logo-mark block shrink-0 text-white light:text-stone-900',
